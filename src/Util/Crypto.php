...
abstract class Crypto
{
	...
	/**
	 * Hash any string value with specified algorithm. It supports hashing with the following algorithms: bcypto, 2y,
	 * argon2i, argon2id. There are defined constant for each type with ALGO prefix in Crypto class.
	 *
	...
	{
		$allowed = [
			self::ALGO_BCRYPT,  self::ALGO_2Y,
			self::ALGO_ARGON2I, self::ALGO_ARGONI2ID
		];

		if (!in_array($algo, $allowed)) {
			throw new InvalidArgumentException('Invalid argument. Allowed algorithms are: bcypto, 2y, argon2i, argon2id');
		}
